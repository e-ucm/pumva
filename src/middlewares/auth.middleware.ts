    ...
    try {
      if (!this.initialized) {
        ...
        try {
          ...
          logger.debug(`[AUTH] JWT validation successful for user: ${result.sql.username}`);
          // Attach user data to result
          
          
          logger.info((req as any).user, `[AUTH] User authenticated: ${result.sql.username}`);
          //Get user from database
          var user : Partial<InstanceType<typeof db.Tables.User>> | null = null;
          try {
            logger.debug(`[AUTH] Looking up user in database: ${result.sql.username}`);
              user = await getOrCreateUserByUsername({ username : result.sql.username, email: result.data.email, role : Authenticator.getRoleFromRealmAccessRoles(result.data) });
          } catch (e) {
